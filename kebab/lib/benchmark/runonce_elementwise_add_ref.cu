...
/**
 * @brief Single-run CUDA baseline elementwise_add kernel for profiling
 * 
 * This binary runs the CUDA baseline elementwise_add kernel exactly once (no loops, no warmup).
 * Designed for use with Nsight Compute profiling to compare against CuTe kernels.
 * 
 * Usage: ./runonce_elementwise_add_ref
 * 
 * Configuration is read from config.yaml
 */
template<typename T>
void runOnceElementwiseAddRef(const ConfigParser& config) {
    std::string type_name = std::is_same_v<T, float> ? "float" : "half";
    std::cout << "\n========================================" << std::endl;
    std::cout << "Single-Run CUDA Baseline Elementwise Add Kernel (" << type_name << ")" << std::endl;
    std::cout << "========================================" << std::endl;
    ...
    std::cout << "  Implementation: CUDA baseline (reference)" << std::endl;
    ...
    std::cout << "Running CUDA baseline kernel once..." << std::endl;
    ...
    std::cout << "CUDA baseline kernel execution complete!" << std::endl;
...
int main() {
    std::cout << "========================================" << std::endl;
    std::cout << "Single-Run CUDA Baseline Elementwise Add Profiling Binary" << std::endl;
    std::cout << "========================================" << std::endl;
    ...
    try {
        ...
        for (const auto& precision : precisions) {
            if (precision == "float32" || precision == "float") {
                runOnceElementwiseAddRef<float>(config);
            } else if (precision == "float16" || precision == "half") {
                if (has_tensor_cores) {
                    runOnceElementwiseAddRef<__half>(config);
                } else {

 ...
 * - Larger tiles: 64×128×64 (vs V2's 64×64×64)
...
template<int ScaleD, int ScaleA, int ScaleB, int TransA, int TransB>
__device__ void wgmma256_v3(float d[16][8], __half* sA, __half* sB) {
    uint64_t desc_a = make_smem_desc_v3(&sA[0]);
    uint64_t desc_b = make_smem_desc_v3(&sB[0]);
    asm volatile(
        "{\n"
...
__device__ inline void wgmma_v3(float d[WGMMA_N/16][8], __half* sA, __half* sB) {
    static_assert(WGMMA_N == 64 || WGMMA_N == 128 || WGMMA_N == 256);
    if constexpr (WGMMA_N == 256)
        wgmma256_v3<ScaleD, ScaleA, ScaleB, TransA, TransB>(d, sA, sB);
...
constexpr int V3_NUM_THREADS = 128;  // 2 warp group
...
template<int ScaleD, int ScaleA, int ScaleB, int TransA, int TransB>
__device__ void wgmma256_v3_bf16(float d[16][8], __nv_bfloat16* sA, __nv_bfloat16* sB) {
    uint64_t desc_a = make_smem_desc_v3_bf16(&sA[0]);
    uint64_t desc_b = make_smem_desc_v3_bf16(&sB[0]);
    asm volatile(
        "{\n"
...
__device__ inline void wgmma_v3_bf16(float d[WGMMA_N/16][8], __nv_bfloat16* sA, __nv_bfloat16* sB) {
    static_assert(WGMMA_N == 64 || WGMMA_N == 128 || WGMMA_N == 256);
    if constexpr (WGMMA_N == 256)
        wgmma256_v3_bf16<ScaleD, ScaleA, ScaleB, TransA, TransB>(d, sA, sB);
    if constexpr (WGMMA_N == 128)

/**
 * @file cuda_gemm_v9_wgmma_tma_warpgroup_warpspecialized_persistent_tilescheduler_ptxbarrier_tma5d_cluster_multicast_streamstore.cu
 * @brief CUDA V9 GEMM with Streaming Stores (based on fast.cu kernel 9)
 *
 * Key features:
 * - Same as V8 but with __stwt streaming stores for output
 * - Streaming stores bypass L2 cache for better memory bandwidth
...
// ============================================================================
// WGMMA Helper Functions (V9)
// ============================================================================

__device__ static inline uint64_t matrix_descriptor_encode_v9(uint64_t x) {
    return (((x) & 0x3FFFF) >> 0x4);
}

__device__ uint64_t make_smem_desc_v9(__half* ptr) {
    uint32_t addr = static_cast<uint32_t>(__cvta_generic_to_shared(ptr));
    uint64_t desc = 0x0000000000000000;
    desc |= matrix_descriptor_encode_v9(addr);
    desc |= matrix_descriptor_encode_v9((uint64_t)16) << 16;
    desc |= matrix_descriptor_encode_v9((uint64_t)1024) << 32;
    desc |= 1llu << 62;
    return desc;
}

__device__ void warpgroup_arrive_v9() {
    asm volatile("wgmma.fence.sync.aligned;\n" ::: "memory");
}

__device__ void warpgroup_commit_batch_v9() {
    asm volatile("wgmma.commit_group.sync.aligned;\n" ::: "memory");
}

template <int N>
__device__ void warpgroup_wait_v9() {
    static_assert(N >= 0 && N <= 7, "WGMMA wait: N must be in range [0, 7]");
    asm volatile("wgmma.wait_group.sync.aligned %0;\n" ::"n"(N) : "memory");
}

template <uint32_t RegCount>
__device__ void warpgroup_reg_alloc_v9() {
    asm volatile("setmaxnreg.inc.sync.aligned.u32 %0;\n" : : "n"(RegCount));
}

template <uint32_t RegCount>
__device__ void warpgroup_reg_dealloc_v9() {
    asm volatile("setmaxnreg.dec.sync.aligned.u32 %0;\n" : : "n"(RegCount));
}

// WGMMA 64×256×16 for FP16
template<int ScaleD, int ScaleA, int ScaleB, int TransA, int TransB>
__device__ void wgmma256_v9(float d[16][8], __half* sA, __half* sB) {
    uint64_t desc_a = make_smem_desc_v9(&sA[0]);
    uint64_t desc_b = make_smem_desc_v9(&sB[0]);
    asm volatile(
        "{\n"
...
// ============================================================================
// PTX Barrier Functions (V9)
// ============================================================================

__device__ static __forceinline__ void init_barrier_v9(uint64_t* bar, int thread_count, int transaction_count) {
...
__device__ static __forceinline__ void expect_bytes_v9(uint64_t* bar, uint32_t bytes) {
    uint32_t bar_ptr = static_cast<uint32_t>(__cvta_generic_to_shared(bar));
    asm volatile(
...
__device__ static inline void load_async_v9(__half *dst, void const* const src_tma_map, uint64_t* bar,
...
__device__ static inline void load_async_multicast_v9(__half *dst, void const* const src_tma_map, uint64_t* bar,
...
__device__ static __forceinline__ void wait_v9(uint64_t* bar, int kPhaseBit) {
    uint32_t mbar_ptr = static_cast<uint32_t>(__cvta_generic_to_shared(bar));
    asm volatile(
        "{\n"
        ".reg .pred P1;\n"
...
__device__ static __forceinline__ void arrive_v9(uint64_t* bar, uint32_t count = 1) {
    uint32_t mbar_ptr = static_cast<uint32_t>(__cvta_generic_to_shared(bar));
    asm volatile(
...
__device__ void arrive_cluster_v9(uint64_t* bar, uint32_t cta_id, uint32_t count = 1) {
...
// ============================================================================
// TMA Functions (3D tensor maps for V9)
// ============================================================================

template <int BlockMajorSize, int BlockMinorSize>
CUtensorMap create_tensor_map_v9(__half* gmem_ptr, int global_height, int global_width) {
    CUtensorMap tma_map;
...
// ============================================================================
// V9 Kernel Configuration
// ============================================================================

constexpr int V9_BM = 128;
constexpr int V9_BN = 256;
constexpr int V9_BK = 64;
constexpr int V9_NUM_THREADS = 384;
constexpr int V9_QSIZE = 3;
constexpr int V9_NUM_SM = 128;
constexpr int V9_CLUSTER_M = 2;
constexpr int V9_CLUSTER_N = 1;
...
#define CEIL_DIV_V9(a, b) (((a) + (b) - 1) / (b))

// Simple linear schedule for V9
template<int NUM_SM, int BM, int BN>
struct ScheduleV9 {
    int st, en;
    int total_blocks_n;

    __device__ __forceinline__ ScheduleV9(int M, int N, int block) {
...
// ============================================================================
// V9 Kernel: Streaming Stores + Clusters + TMA Multicast
// ============================================================================
...
gemm_v9_streamstore_kernel(int M, int N, int K, __half* C,
    ...
    SMemV9<BM, BN, BK, QSIZE> &s = *reinterpret_cast<SMemV9<BM, BN, BK, QSIZE>*>(smem);
    __half *sA = s.A;
    __half *sB = s.B;
    ...
    if (threadIdx.x == 0) {
        for (int i = 0; i < QSIZE; ++i) {
            init_barrier_v9(&full[i], 0, 1);
            init_barrier_v9(&empty[i], 0, num_consumers * CLUSTERS);
        }
    }
    asm volatile("barrier.cluster.arrive;\n" : :);
    asm volatile("barrier.cluster.wait;\n" : :);

    ScheduleV9<NUM_SM / CLUSTERS, BM * CLUSTER_M, BN * CLUSTER_N> schedule(M, N, rank);
    ...
    if (wg_idx == 0) {
        ...
        warpgroup_reg_dealloc_v9<num_regs>();
        if (tid == 0) {
            int p = 0;
            int qidx = 0;
            uint32_t col_mask = 0;
            ...
            while (schedule.next(num_block_m, num_block_n)) {
                ...
                for (int block_k_iter = 0; block_k_iter < num_blocks_k; ++block_k_iter, ++qidx) {
                    ...
                    wait_v9(&empty[qidx], p);

                    expect_bytes_v9(&full[qidx], (BK * BN + BK * BM) * sizeof(__half));
                    if constexpr (CLUSTER_N > 1) {
                        uint32_t mask = ((1 << CLUSTER_N) - 1) << (rank_m * CLUSTER_N);
                        if (rank_n == 0) {
                            load_async_multicast_v9(&sA[qidx * BK * BM], &tensorMapA, &full[qidx],
                                                    block_k_iter * BK, num_block_m * BM, mask);
                        }
                    } else {
                        load_async_v9(&sA[qidx * BK * BM], &tensorMapA, &full[qidx],
                                      block_k_iter * BK, num_block_m * BM);
                    }

                    if constexpr (CLUSTER_M > 1) {
                        if (rank_m == 0) {
                            load_async_multicast_v9(&sB[qidx * BK * BN], &tensorMapB, &full[qidx],
                                                    block_k_iter * BK, num_block_n * BN, col_mask);
                        }
                    } else {
                        load_async_v9(&sB[qidx * BK * BN], &tensorMapB, &full[qidx],
    ...
    // Consumer warp-groups
        ...
        warpgroup_reg_alloc_v9<num_regs>();
        float d[B_WG_M / WGMMA_M][WGMMA_N / 16][8];
        --wg_idx;

        for (int qidx = 0; qidx < QSIZE; ++qidx) {
            if (tid < CLUSTERS) arrive_cluster_v9(&empty[qidx], tid);
        }
        ...
        while (schedule.next(num_block_m, num_block_n)) {
            ...
            for (int block_k_iter = 0; block_k_iter < num_blocks_k; ++block_k_iter, ++qidx) {
                ...
                for (int m_it = 0; m_it < B_WG_M / WGMMA_M; ++m_it) {
                    ...
                    for (int bk = 0; bk < BK; bk += 64) {
                        ...
                        for (int k_it = 0; k_it < 64 / WGMMA_K; ++k_it) {
                            wgmma256_v9<1, 1, 1, 0, 0>(d[m_it], &wgmma_sA[k_it * WGMMA_K],
                ...
                warpgroup_commit_batch_v9();
                warpgroup_wait_v9<0>();
                if (tid < CLUSTERS) arrive_cluster_v9(&empty[qidx], tid);
            }

            // Store results using streaming stores (__stwt)
...
static CUtensorMap v9_tma_map_A;
...
void gemm_v9_wgmma_tma_warpgroup_warpspecialized_persistent_tilescheduler_ptxbarrier_tma5d_cluster_multicast_streamstore_fp16(
    ...
    constexpr int CLUSTER_BM = V9_BM * V9_CLUSTER_M;
    constexpr int CLUSTER_BN = V9_BN * V9_CLUSTER_N;
    if (M % CLUSTER_BM != 0 || N % CLUSTER_BN != 0 || K % V9_BK != 0) {
        fprintf(stderr, "ERROR: CUDA V9 requires M%%256==0, N%%256==0, K%%64==0\n");
        return;
    }

    if (M != v9_prev_m || N != v9_prev_n || K != v9_prev_k) {
        v9_tma_map_A = create_tensor_map_v9<V9_BM, V9_BK>(const_cast<__half*>(A), M, K);
        v9_tma_map_B = create_tensor_map_v9<V9_BN, V9_BK>(const_cast<__half*>(B), N, K);
        v9_prev_m = M; v9_prev_n = N; v9_prev_k = K;
    }

    size_t sMemSize = sizeof(SMemV9<V9_BM, V9_BN, V9_BK, V9_QSIZE>);
    auto kernel = gemm_v9_streamstore_kernel<V9_BM, V9_BN, V9_BK, V9_NUM_THREADS, V9_QSIZE, V9_NUM_SM, V9_CLUSTER_M, V9_CLUSTER_N>;
    cudaFuncSetAttribute(kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, sMemSize);

    kernel<<<V9_NUM_SM, V9_NUM_THREADS, sMemSize, stream>>>(
        M, N, K, const_cast<__half*>(C), v9_tma_map_A, v9_tma_map_B);
}

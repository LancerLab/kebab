...
// ============================================================================
// BFloat16 Version of V12
// ============================================================================

__device__ uint64_t make_smem_desc_v12_bf16(__nv_bfloat16* ptr) {
    uint32_t addr = static_cast<uint32_t>(__cvta_generic_to_shared(ptr));
    uint64_t desc = 0x0000000000000000;
    desc |= matrix_descriptor_encode_v12(addr);
    desc |= matrix_descriptor_encode_v12((uint64_t)16) << 16;
...
// WGMMA 64×256×16 for BF16 (FP32 accumulation)
template<int ScaleD, int ScaleA, int ScaleB, int TransA, int TransB>
__device__ void wgmma256_v12_bf16(float d[16][8], __nv_bfloat16* sA, __nv_bfloat16* sB) {
    uint64_t desc_a = make_smem_desc_v12_bf16(&sA[0]);
    uint64_t desc_b = make_smem_desc_v12_bf16(&sB[0]);
    asm volatile(
        "{\n"
...
// TMA functions for BF16
__device__ static inline void load_async_v12_bf16(__nv_bfloat16 *dst, void const* const src_tma_map, uint64_t* bar,
                                                   int global_col_idx, int global_row_idx) {
    uint64_t tma_ptr = reinterpret_cast<uint64_t>(src_tma_map);
    uint32_t mbar_ptr = static_cast<uint32_t>(__cvta_generic_to_shared(bar));
    uint32_t dst_ptr = static_cast<uint32_t>(__cvta_generic_to_shared(dst));
...
__device__ static inline void load_async_multicast_v12_bf16(__nv_bfloat16 *dst, void const* const src_tma_map, uint64_t* bar,
                                                             int global_col_idx, int global_row_idx, uint16_t cluster_mask) {
    uint64_t tma_ptr = reinterpret_cast<uint64_t>(src_tma_map);
    uint32_t mbar_ptr = static_cast<uint32_t>(__cvta_generic_to_shared(bar));
    uint32_t dst_ptr = static_cast<uint32_t>(__cvta_generic_to_shared(dst));
...
__device__ static inline void store_async_v12_bf16(void const* dst_tma_map, __nv_bfloat16 *src, int global_col_idx, int global_row_idx) {
    uint64_t tma_ptr = reinterpret_cast<uint64_t>(dst_tma_map);
    uint32_t src_ptr = static_cast<uint32_t>(__cvta_generic_to_shared(src));
    asm volatile(
        "cp.async.bulk.tensor.3d.global.shared::cta.tile.bulk_group [%0, {%2, %3, %4}], [%1];"
...
template <int BlockMajorSize, int BlockMinorSize, bool swizzle = true, bool padding = false>
CUtensorMap create_tensor_map_v12_bf16(__nv_bfloat16* gmem_ptr, int global_height, int global_width) {
    CUtensorMap tma_map;
    void* gmem_address = (void*)gmem_ptr;
    uint64_t gmem_prob_shape[5] = {64, (uint64_t)global_height, (uint64_t)global_width / 64, 1, 1};
...
template<int BM, int BN, int BK, int NUM_THREADS, int QSIZE, int NUM_SM, int CLUSTER_M, int CLUSTER_N>
__global__ void __launch_bounds__(NUM_THREADS) __cluster_dims__(CLUSTER_M * CLUSTER_N, 1, 1)
gemm_v12_stmatrix_kernel_bf16(int M, int N, int K,
                               const __grid_constant__ CUtensorMap tensorMapC,
                               const __grid_constant__ CUtensorMap tensorMapA,
                               const __grid_constant__ CUtensorMap tensorMapB,
                               int* dspace) {
    ...
    extern __shared__ __align__(128) uint8_t smem[];
    SMemV12_BF16<BM, BN, BK, QSIZE> &s = *reinterpret_cast<SMemV12_BF16<BM, BN, BK, QSIZE>*>(smem);
    ...
    if (wg_idx == 0) {
        ...
        if (tid == 0) {
            ...
            while (schedule.next(num_block_m, num_block_n)) {
                ...
                for (int block_k_iter = 0; block_k_iter < num_blocks_k; ++block_k_iter, ++qidx) {
                    if (qidx == QSIZE) { qidx = 0; p ^= 1; }
                    wait_v12(&empty[qidx], p);
                    expect_bytes_v12(&full[qidx], (BK * BN + BK * BM) * sizeof(__nv_bfloat16));
                    if constexpr (CLUSTER_N > 1) {
                        uint32_t mask = ((1 << CLUSTER_N) - 1) << (rank_m * CLUSTER_N);
                        if (rank_n == 0) load_async_multicast_v12_bf16(&sA[qidx * BK * BM], &tensorMapA, &full[qidx], block_k_iter * BK, num_block_m * BM, mask);
                    } else {
                        load_async_v12_bf16(&sA[qidx * BK * BM], &tensorMapA, &full[qidx], block_k_iter * BK, num_block_m * BM);
                    }
                    if constexpr (CLUSTER_M > 1) {
                        if (rank_m == 0) load_async_multicast_v12_bf16(&sB[qidx * BK * BN], &tensorMapB, &full[qidx], block_k_iter * BK, num_block_n * BN, col_mask);
                    } else {
                        load_async_v12_bf16(&sB[qidx * BK * BN], &tensorMapB, &full[qidx], block_k_iter * BK, num_block_n * BN);
                    }
                }
            }
        }
        ...
        int p = 0, qidx = 0;
        int lane = tid % 32, warp = tid / 32;
        __nv_bfloat16* block_sC = sC + wg_idx * B_WG_M_PADDED * BN;
        uint32_t tid_offset = warp * 16 + (lane % 8) * B_WG_M_PADDED;
        tid_offset += (lane / 16) * B_WG_M_PADDED * 8 + (lane & 8);
        uint32_t base_addr = static_cast<uint32_t>(__cvta_generic_to_shared(block_sC)) + tid_offset * sizeof(__nv_bfloat16);
        ...
        while (schedule.next(num_block_m, num_block_n)) {
            ...
            // Use stmatrix for efficient shared memory stores
            __nv_bfloat16 d_bf16[8];
            int* data_ptr = (int*)d_bf16;
            for (int m_it = 0; m_it < B_WG_M / WGMMA_M; ++m_it) {
                for (int w = 0; w < WGMMA_N; w += 16) {
                    uint32_t addr = base_addr + (w * B_WG_M_PADDED + m_it * WGMMA_M) * sizeof(__nv_bfloat16);
                    for (int k = 0; k < 8; k++) d_bf16[k] = __float2bfloat16(d[m_it][w / 16][k]);
                    asm volatile("stmatrix.sync.aligned.m8n8.x4.trans.shared::cta.b16 [%0], {%1, %2, %3, %4};"
                                :: "r"(addr), "r"(data_ptr[0]), "r"(data_ptr[1]), "r"(data_ptr[2]), "r"(data_ptr[3]));
                }
            }
            asm volatile("bar.sync %0, 128;\n" :: "r"(wg_idx + 2) : "memory");
            if (tid == 0) {
                store_async_v12_bf16(&tensorMapC, block_sC, num_block_m * BM + wg_idx * B_WG_M, num_block_n * BN);
                asm volatile("cp.async.bulk.commit_group;");
            }
        }
    }
...
// BF16 Host Function
static CUtensorMap v12_tma_map_A_bf16, v12_tma_map_B_bf16, v12_tma_map_C_bf16;
static int v12_prev_m_bf16 = 0, v12_prev_n_bf16 = 0, v12_prev_k_bf16 = 0;
static int* v12_dspace_bf16 = nullptr;

void gemm_v12_stmatrix_bf16(const __nv_bfloat16* A, const __nv_bfloat16* B, __nv_bfloat16* C,
                            int M, int N, int K, char lhs_format, char rhs_format,
                            cudaStream_t stream) {
    if (lhs_format != 'R' || rhs_format != 'C') {
        fprintf(stderr, "ERROR: CUDA V12 BF16 only supports RC mode\n");
        return;
    }
    constexpr int CLUSTER_BM = V12_BM * V12_CLUSTER_M;
    constexpr int CLUSTER_BN = V12_BN * V12_CLUSTER_N;
    if (M % CLUSTER_BM != 0 || N % CLUSTER_BN != 0 || K % V12_BK != 0) {
        fprintf(stderr, "ERROR: CUDA V12 BF16 requires M%%256==0, N%%256==0, K%%64==0\n");
        return;
    }

    if (M != v12_prev_m_bf16 || N != v12_prev_n_bf16 || K != v12_prev_k_bf16) {
        v12_tma_map_A_bf16 = create_tensor_map_v12_bf16<V12_BM, V12_BK>(const_cast<__nv_bfloat16*>(A), M, K);
        v12_tma_map_B_bf16 = create_tensor_map_v12_bf16<V12_BN, V12_BK>(const_cast<__nv_bfloat16*>(B), N, K);
        v12_tma_map_C_bf16 = create_tensor_map_v12_bf16<V12_BN, V12_B_WG_M, false, true>(const_cast<__nv_bfloat16*>(C), N, M);
        v12_prev_m_bf16 = M; v12_prev_n_bf16 = N; v12_prev_k_bf16 = K;

        int* space = (int*)malloc(sizeof(int) * V12_NUM_SM * V12_SPACE_LEN);
        createHilbert_v12(M / V12_BM / V12_CLUSTER_M, N / V12_BN / V12_CLUSTER_N,
                          V12_NUM_SM / V12_CLUSTER_M / V12_CLUSTER_N, space);
        if (v12_dspace_bf16) cudaFree(v12_dspace_bf16);
        cudaMalloc((void**)&v12_dspace_bf16, sizeof(int) * V12_NUM_SM * V12_SPACE_LEN);
        cudaMemcpy(v12_dspace_bf16, space, sizeof(int) * V12_NUM_SM * V12_SPACE_LEN, cudaMemcpyHostToDevice);
        free(space);
    }

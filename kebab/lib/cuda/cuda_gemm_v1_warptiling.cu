...
__global__ void __launch_bounds__(NUM_THREADS)
gemm_v1_warptiling_rc_kernel_bf16(int M, int N, int K,
    ...
    const uint innerColB_cm = threadIdx.x / (BK / 4);
    const uint innerRowB_cm = threadIdx.x % (BK / 4);
    constexpr uint colStrideB = (NUM_THREADS * 4) / BK;

    float threadResults[WMITER * TM * WNITER * TN] = {0.0f};
    __nv_bfloat16 regM[WMITER * TM];
    __nv_bfloat16 regN[WNITER * TN];
    ...
    for (uint bkIdx = 0; bkIdx < K; bkIdx += BK) {
        for (uint offset = 0; offset + rowStrideA <= BM; offset += rowStrideA) {
            ...
            const float2 x = *reinterpret_cast<const float2*>(
                &A[(innerRowA + offset) * K + innerColA * 4]);
            memcpy(&tmp[0], &x, sizeof(__nv_bfloat16) * 4);
            As[(innerColA * 4 + 0) * BM + innerRowA + offset] = tmp[0];
            As[(innerColA * 4 + 1) * BM + innerRowA + offset] = tmp[1];
            As[(innerColA * 4 + 2) * BM + innerRowA + offset] = tmp[2];
            As[(innerColA * 4 + 3) * BM + innerRowA + offset] = tmp[3];
        ...
        for (uint wSubColIdx = 0; wSubColIdx < WNITER; ++wSubColIdx) {
            for (uint resIdxM = 0; resIdxM < TM; ++resIdxM) {
                for (uint resIdxN = 0; resIdxN < TN; ++resIdxN) {
                    ...
                    const int i = (wSubRowIdx * TM + resIdxM) * (WNITER * TN) +
                                  wSubColIdx * TN + resIdxN;
                    C[globalCol * M + globalRow] = __float2bfloat16(threadResults[i]);
                }
            }
        }
    }
...
void gemm_v1_warptiling_bf16(const __nv_bfloat16* A, const __nv_bfloat16* B, __nv_bfloat16* C,
                              ...
                              cudaStream_t stream) {
    if (lhs_format != 'R' || rhs_format != 'C') {
        fprintf(stderr, "ERROR: CUDA V1 BF16 (warp tiling) only supports RC mode\n");
        return;
    }
    dim3 gridDim((N + BN - 1) / BN, (M + BM - 1) / BM);
    gemm_v1_warptiling_rc_kernel_bf16<<<gridDim, NUM_THREADS, 0, stream>>>(M, N, K, A, B, C);
}

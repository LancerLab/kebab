...
void
gemm_device_tma_v3(ProblemShape shape_MNK, CtaTiler cta_tiler,
                ...
                Alpha alpha, Beta beta)
{
  ...
  using SharedStorage = SharedStorageV3<TA, TB, SmemLayoutA, SmemLayoutB>;
...
void
gemm_nt_tma_v3(int m, int n, int k,
            Alpha alpha,
            TA const* A, int ldA,
            TB const* B, int ldB,
            Beta beta,
            ...
            cudaStream_t stream = 0)
{
  ...
  int smem_size = int(sizeof(SharedStorageV3<TA, TB, decltype(sA), decltype(sB)>));
  ...
  // For small matrices, use 1x1 cluster to avoid launching unnecessary blocks
  dim3 dimCluster(
    (grid_m >= 2) ? 2 : 1,
    (grid_n >= 2) ? 2 : 1,
    1
  ...
  void const* kernel_ptr = reinterpret_cast<void const*>(
                              &gemm_device_tma_v3<decltype(prob_shape), decltype(cta_tiler),
...
template <class TA, class TB, class TC,
          ...
          int BLK_M = 128, int BLK_N = 128, int BLK_K = 64>
void
gemm_tn_tma_v3(int m, int n, int k,
            Alpha alpha,
            TA const* A, int ldA,
            TB const* B, int ldB,
            Beta beta,
...
template <class TA, class TB, class TC,
          ...
          int BLK_M = 128, int BLK_N = 128, int BLK_K = 64>
void
gemm_tt_tma_v3(int m, int n, int k,
            Alpha alpha,
            TA const* A, int ldA,
            TB const* B, int ldB,
            Beta beta,
...
template <class TA, class TB, class TC,
          ...
          int BLK_M = 128, int BLK_N = 128, int BLK_K = 64>
void
gemm_nn_tma_v3(int m, int n, int k,
            Alpha alpha,
            TA const* A, int ldA,
            TB const* B, int ldB,
            Beta beta,
 ...
 */
void gemm_wgmma_tma_v3_fp16_dispatch(const void* A_ptr, const void* B_ptr, void* C_ptr,
                                  int M, int N, int K, 
                                  ...
                                  cudaStream_t stream)
{
    ...
    if (lhs_format == 'R' && rhs_format == 'C') {
        ...
        if (tile_M == 128 && tile_N == 128 && tile_K == 64) {
            gemm_tn_tma_v3<half_t, half_t, half_t, float, float, 128, 128, 64>(
                M, N, K, alpha, A, K, B, K, beta, C, M, stream);
        } else if (tile_M == 256 && tile_N == 128 && tile_K == 64) {
            gemm_tn_tma_v3<half_t, half_t, half_t, float, float, 256, 128, 64>(
                M, N, K, alpha, A, K, B, K, beta, C, M, stream);
        } else if (tile_M == 128 && tile_N == 256 && tile_K == 64) {
            gemm_tn_tma_v3<half_t, half_t, half_t, float, float, 128, 256, 64>(
                M, N, K, alpha, A, K, B, K, beta, C, M, stream);
        } else {
        ...
        if (tile_M == 64 && tile_N == 64 && tile_K == 32) {
            ...
            fprintf(stderr, "Warning: Tile size [%d,%d,%d] may not be supported by TMA TN due to GMMA constraints, using default [128,128,64]\n", 
                    tile_M, tile_N, tile_K);
            gemm_nt_tma_v3<half_t, half_t, half_t, float, float, 128, 128, 64>(
                M, N, K, alpha, A, M, B, N, beta, C, M, stream);
        } else if (tile_M == 128 && tile_N == 128 && tile_K == 64) {
            gemm_nt_tma_v3<half_t, half_t, half_t, float, float, 128, 128, 64>(
                M, N, K, alpha, A, M, B, N, beta, C, M, stream);
        } else if (tile_M == 256 && tile_N == 128 && tile_K == 64) {
            gemm_nt_tma_v3<half_t, half_t, half_t, float, float, 256, 128, 64>(
                M, N, K, alpha, A, M, B, N, beta, C, M, stream);
        } else if (tile_M == 128 && tile_N == 256 && tile_K == 64) {
            gemm_nt_tma_v3<half_t, half_t, half_t, float, float, 128, 256, 64>(
                M, N, K, alpha, A, M, B, N, beta, C, M, stream);
        } else {
            // Default fallback
        ...
        // Dispatch based on tile configuration
        if (tile_M == 128 && tile_N == 128 && tile_K == 64) {
            gemm_tt_tma_v3<half_t, half_t, half_t, float, float, 128, 128, 64>(
                M, N, K, alpha, A, K, B, N, beta, C, M, stream);
        } else if (tile_M == 256 && tile_N == 128 && tile_K == 64) {
            gemm_tt_tma_v3<half_t, half_t, half_t, float, float, 256, 128, 64>(
                M, N, K, alpha, A, K, B, N, beta, C, M, stream);
        } else if (tile_M == 128 && tile_N == 256 && tile_K == 64) {
            gemm_tt_tma_v3<half_t, half_t, half_t, float, float, 128, 256, 64>(
                M, N, K, alpha, A, K, B, N, beta, C, M, stream);
        } else {
            if (tile_K != 64) {
                fprintf(stderr, "Warning: TMA requires K=64 (got K=%d), using default [128,128,64]\n", tile_K);
        ...
        // Dispatch based on tile configuration
        if (tile_M == 128 && tile_N == 128 && tile_K == 64) {
            gemm_nn_tma_v3<half_t, half_t, half_t, float, float, 128, 128, 64>(
                M, N, K, alpha, A, M, B, K, beta, C, M, stream);
        } else if (tile_M == 256 && tile_N == 128 && tile_K == 64) {
            gemm_nn_tma_v3<half_t, half_t, half_t, float, float, 256, 128, 64>(
                M, N, K, alpha, A, M, B, K, beta, C, M, stream);
        } else if (tile_M == 128 && tile_N == 256 && tile_K == 64) {
            gemm_nn_tma_v3<half_t, half_t, half_t, float, float, 128, 256, 64>(
                M, N, K, alpha, A, M, B, K, beta, C, M, stream);
        } else {
            if (tile_K != 64) {
                fprintf(stderr, "Warning: TMA requires K=64 (got K=%d), using default [128,128,64]\n", tile_K);
 ...
 */
void gemm_wgmma_tma_v3_fp16(const void* A_ptr, const void* B_ptr, void* C_ptr,
                         int M, int N, int K,
                         char lhs_format, char rhs_format,
                         cudaStream_t stream)
{
    ...
    gemm_wgmma_tma_v3_fp16_dispatch(A_ptr, B_ptr, C_ptr, M, N, K,
                                lhs_format, rhs_format,
